...
{
    [Fact]
    public async Task DirectManagerRequestProducesFinalAnswerAndGovernedProjectProposal()
    {
        ...
        var runtime = new AgentTestRuntime()
            .RegisterCapability<JsonElement, JsonElement>(PlatformCapabilities.LlmChatStream,
                (_, _) => Task.FromResult(JsonSerializer.SerializeToElement(new {
                    text = """{"response":"I will scope a playable breakout demo. Hire a game engineer first for the loop and effects, then QA for validation.","createProject":true,"projectName":"Breakout Demo","projectOutcome":"Build a playable breakout demo with appealing special effects.","teamRoles":[{"roleKey":"game-engineer","title":"Game Engineer","purpose":"Implement the breakout loop and effects.","priority":1},{"roleKey":"game-quality-assurance","title":"Game QA","purpose":"Validate the playable demo.","priority":2}]}""",
                    role = "assistant"
                })))
        ...
        Assert.Contains(runtime.Progress, x => x.GetProperty("isFinal").GetBoolean()
            ...
            && x.GetProperty("delta").GetString()!.Contains("game engineer"));
    }
